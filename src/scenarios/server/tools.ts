        ...
        try {
            ...
            if (!result.tools) {
                ...
                result.tools.forEach((tool, index) => {
                    ...
                    if (!tool.inputSchema) errors.push(`Tool ${index}: missing inputSchema`);
                });
            }
...
export class ToolsCallSimpleTextScenario implements ClientScenario {
    ...
    async run(serverUrl: string): Promise<ConformanceCheck[]> {
        ...
        try {
            ...
            if (content && content.length === 0) errors.push('content array is empty');

            const textContent = content && content.find((c: any) => c.type === 'text');
            if (!textContent) errors.push('No text content found');
            if (textContent && !textContent.text) errors.push('Text content missing text field');
...
export class ToolsCallImageScenario implements ClientScenario {
    ...
    async run(serverUrl: string): Promise<ConformanceCheck[]> {
        ...
        try {
            ...
            const imageContent = content && content.find((c: any) => c.type === 'image');
            if (!imageContent) errors.push('No image content found');
            if (imageContent && !imageContent.data) errors.push('Image content missing data field');
            if (imageContent && !imageContent.mimeType) errors.push('Image content missing mimeType');
...
export class ToolsCallMultipleContentTypesScenario implements ClientScenario {
    ...
    async run(serverUrl: string): Promise<ConformanceCheck[]> {
        ...
        try {
            ...
            const content = (result as any).content;
            if (!content) errors.push('Missing content array');
            if (content && content.length < 2) errors.push('Expected multiple content items');

            const hasText = content && content.some((c: any) => c.type === 'text');
            const hasImage = content && content.some((c: any) => c.type === 'image');
            const hasResource = content && content.some((c: any) => c.type === 'resource');
    ...
    async run(serverUrl: string): Promise<ConformanceCheck[]> {
        ...
        try {
            ...
            if (logNotifications.length === 0) {
                ...
                errors.push(`Expected at least 3 log messages, got ${logNotifications.length}`);
            }
    ...
    async run(serverUrl: string): Promise<ConformanceCheck[]> {
        ...
        try {
            ...
            if (progressUpdates.length === 0) {
                ...
                errors.push(`Expected at least 3 progress notifications, got ${progressUpdates.length}`);
            }
...
export class ToolsCallAudioScenario implements ClientScenario {
    ...
    async run(serverUrl: string): Promise<ConformanceCheck[]> {
        ...
        try {
            ...
            const audioContent = content && content.find((c: any) => c.type === 'audio');
...
export class ToolsCallEmbeddedResourceScenario implements ClientScenario {
    ...
    async run(serverUrl: string): Promise<ConformanceCheck[]> {
        ...
        try {
            ...
            const resourceContent = content && content.find((c: any) => c.type === 'resource');
            if (!resourceContent) errors.push('No resource content found');
            if (resourceContent && !resourceContent.resource) errors.push('Resource content missing resource field');
            if (resourceContent?.resource) {
                if (!resourceContent.resource.uri) errors.push('Resource missing uri field');
                if (!resourceContent.resource.mimeType) errors.push('Resource missing mimeType field');
